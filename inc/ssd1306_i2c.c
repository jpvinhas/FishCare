...
{
    // Novos caracteres – ajuste os índices conforme a ordem no array font
    if (character == '!')
        return 37;  // índice para '!'
    else if (character == ':')
        return 38;  // índice para ':'
    else if (character == ')')
        return 39;  // índice para ')'
    else if (character == 'º')
        return 40;  // índice para 'º'
    else if (character == '-')
        return 41;  // índice para '-'
    else if (character == '.')
        return 42;  // índice para '.'
    
    // Mapeamento original para letras (A–Z)
    else if (character >= 'A' && character <= 'Z')
        return character - 'A' + 1;
    // Mapeamento para dígitos (0–9)
    else if (character >= '0' && character <= '9')
        return character - '0' + 27;
    else
        return 0;  // caractere não mapeado retorna "Nothing"
}
